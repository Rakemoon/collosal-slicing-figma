...
export default function Footer({ list }: FooterProps) {
    return <footer className="grid grid-cols-12 gap-10 w-fit">
        ...
        </div>
        {
            list.map(x => <ul key={x.title} className="col-span-3 text-base">
                <h4 className="text-sm font-semibold mb-5">{x.title}</h4>
                {
                    x.items.map(x => <li key={x.name}><Link href={x.href} className="font-light">{x.name}</Link></li>)
                }
            </ul>
            )
        }
